...
bool notMoved(const vector<int>& pos, const vector<int>& npos)
{
...
// return true if it is solvable in count moves
bool solve(vector<vector<char>> board, int count, vector<int> pos) {
	...
	for (int d = 0; d < 4; d++) {
		...
		if (success(npos)) { return true; }
		else if ((!fail(npos)) && !notMoved(pos, npos)) // moved, but not success
		{
			if (solve(nboard, count - 1, npos)) { return true; }
		}
	}
...
int main() {
	...
	if (solve(board, 10, pos)) { cout << 1 << endl; }
	else { cout << 0 << endl; }
